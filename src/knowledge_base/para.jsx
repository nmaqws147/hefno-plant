...
import diseasesData from '../disease-folder/para.json';

const paraData = diseasesData.groups?.find(g => g.group_name_en === 'Parasitic Plants')?.pathogens || [];
...
const getHostsList = (d) => {
  if (!d.hostPlants) return '-';
  if (Array.isArray(d.hostPlants)) return d.hostPlants.join('، ');
  if (typeof d.hostPlants === 'object') return Object.values(d.hostPlants).flat().join('، ');
  return String(d.hostPlants);
};

const getHostsPreview = (d) => {
  if (!d.hostPlants) return 'متعدد العوائل';
  if (Array.isArray(d.hostPlants)) return d.hostPlants[0];
  if (typeof d.hostPlants === 'object') {
    const vals = Object.values(d.hostPlants).flat();
    return vals[0] || 'متعدد العوائل';
  }
  return String(d.hostPlants);
};
      ...
      <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        ...
        <div className="mb-6 rounded-2xl border border-gray-200/60 dark:border-gray-700/50 bg-white dark:bg-gray-800/80 p-5 sm:p-6 shadow-sm">
          <div className="flex items-center gap-5">
            ...
            <div>
              <h1 className="text-2xl font-black text-gray-900 dark:text-white">النباتات الطفيلية</h1>
              <p className="mt-1 text-sm text-gray-400 dark:text-gray-500 italic">Parasitic Plants</p>
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">النباتات الطفيلية نباتات حقيقية تعيش كلياً أو جزئياً على حساب نباتات أخرى.</p>
              <div className="mt-3 flex flex-wrap gap-2">
                ...
                <span className="inline-flex items-center gap-1.5 rounded-xl border border-gray-100/40 dark:border-gray-800/40 bg-gray-50 dark:bg-gray-800/50 px-3 py-1.5 text-xs font-bold text-gray-700 dark:text-gray-300">
                  Orobanche • Phelipanche • Cuscuta • Striga
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {paginated.map((d) => (
            <div
              key={d.id}
              onClick={() => openModal(d)}
              className="rounded-2xl border border-gray-200/60 dark:border-gray-700/50 bg-white dark:bg-gray-800/70 p-4 shadow-sm hover:-translate-y-1 hover:shadow-md transition-all duration-300 cursor-pointer border-r-[3px] border-red-500"
            >
        ...
        {totalPages > 1 && (
          <div className="mt-8 flex items-center justify-center gap-2">
            <button
              ...
              disabled={page === 1}
              className="flex h-8 w-8 items-center justify-center rounded-xl text-xs font-bold transition-all disabled:opacity-30 enabled:hover:bg-red-50 dark:enabled:hover:bg-red-900/30 text-gray-600 dark:text-gray-400"
            >
          ...
          >
            ...
            >
              <div className="sticky top-0 z-10 flex items-center gap-4 border-b border-gray-100 dark:border-gray-800 bg-white/80 dark:bg-gray-900/80 backdrop-blur-md px-5 py-4">
                <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-red-100 dark:bg-red-900/40 text-red-600 dark:text-red-400 shrink-0 text-xl">
                  <span>🧫</span>
                </div>
              ...
              <div className="p-5 space-y-4">
                    ...
                    <div className="rounded-xl bg-gray-50 dark:bg-gray-800/50 border border-gray-100/60 dark:border-gray-700/30 p-4">
                      <div className="grid grid-cols-2 gap-3 text-xs">
                        ...
                        <div>
                          ...
                          <p className="font-bold text-gray-900 dark:text-white mt-0.5">{getHostsList(selected)}</p>
                        </div>
                      </div>
                    </div>
                ...
                {activeTab === 'management' && (
                  <>
                    {selected.management && (
                      <div className="rounded-xl bg-gray-50 dark:bg-gray-800/50 border border-gray-100/60 dark:border-gray-700/30 p-4">
                        <h3 className="mb-2 flex items-center gap-1.5 text-xs font-bold text-red-700 dark:text-red-300">
                          <Shield size={12} />
                          المكافحة
                        </h3>
