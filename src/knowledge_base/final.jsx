...
import diseasesData from '../disease-folder/final.json';

const pathogens = diseasesData.groups?.find(g => g.group_name_en === 'Final Additions — Remaining Pathogens')?.pathogens || [];
...
const FinalAdditionsPage = () => {
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-gray-900 text-gray-800 dark:text-gray-100 transition-colors duration-300" dir="rtl">
      <SEO title="الإضافات النهائية — باقي المسببات المرضية" description="الإضافات النهائية لبقية المسببات المرضية في قاعدة البيانات." url="/knowledge-base/diseases/final-additions" />

      <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        <button
          onClick={() => navigate('/knowledge-base/diseases')}
          className="mb-5 inline-flex items-center gap-2 rounded-xl bg-violet-50 dark:bg-violet-950/40 px-4 py-2 text-sm font-medium text-violet-700 dark:text-violet-400 transition-colors hover:bg-violet-100 dark:hover:bg-violet-950/80"
        >
        ...
        <div className="mb-6 rounded-2xl border border-gray-200/60 dark:border-gray-700/50 bg-white dark:bg-gray-800/80 p-5 sm:p-6 shadow-sm">
          <div className="flex items-center gap-5">
            <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br from-violet-500 to-purple-600 text-white shadow-lg shadow-violet-200/50 dark:shadow-violet-900/30 shrink-0">
              <span className="text-2xl">🧬</span>
            </div>
            <div>
              <h1 className="text-2xl font-black text-gray-900 dark:text-white">الإضافات النهائية — باقي المسببات</h1>
              <p className="mt-1 text-sm text-gray-400 dark:text-gray-500 italic">Final Additions — Remaining Pathogens</p>
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">بقية المسببات المرضية المكملة لقاعدة البيانات الشاملة.</p>
              <div className="mt-3 flex flex-wrap gap-2">
                <span className="inline-flex items-center gap-1.5 rounded-xl border border-violet-100/40 dark:border-violet-900/40 bg-violet-50 dark:bg-violet-950/40 px-3 py-1.5 text-xs font-bold text-violet-700 dark:text-violet-400">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {paginated.map((d) => (
            <div
              key={d.id}
              onClick={() => openModal(d)}
              className="rounded-2xl border border-gray-200/60 dark:border-gray-700/50 bg-white dark:bg-gray-800/70 p-4 shadow-sm hover:-translate-y-1 hover:shadow-md transition-all duration-300 cursor-pointer border-r-[3px] border-violet-500"
            >
        ...
        {totalPages > 1 && (
          <div className="mt-8 flex items-center justify-center gap-2">
            <button
              ...
              disabled={page === 1}
              className="flex h-8 w-8 items-center justify-center rounded-xl text-xs font-bold transition-all disabled:opacity-30 enabled:hover:bg-violet-50 dark:enabled:hover:bg-violet-900/30 text-gray-600 dark:text-gray-400"
            >
            ...
            {Array.from({ length: totalPages }, (_, i) => i + 1).map(p => (
              <button
                ...
                className={`flex h-8 w-8 items-center justify-center rounded-xl text-xs font-bold transition-all ${
                  page === p
                    ? 'bg-violet-500 text-white shadow-md shadow-violet-200/50 dark:shadow-violet-900/30'
          ...
          >
            ...
            >
              <div className="sticky top-0 z-10 flex items-center gap-4 border-b border-gray-100 dark:border-gray-800 bg-white/80 dark:bg-gray-900/80 backdrop-blur-md px-5 py-4">
                <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-violet-100 dark:bg-violet-900/40 text-violet-600 dark:text-violet-400 shrink-0 text-xl">
                  <span>🧬</span>
                </div>
