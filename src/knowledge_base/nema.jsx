// components/knowledge/NematodesDiseasesPage.jsx
import { useState } from 'react';
import { useNavigate } from 'react-router-dom';
import nema from '../disease-folder/nema.json';
import './nema.css';

const NematodesDiseasesPage = () => {
  const navigate = useNavigate();
  const [selectedDisease, setSelectedDisease] = useState(null);
  const [showModal, setShowModal] = useState(false);

  // استيراد أمراض النيماتودا من ملف البيانات
  const nematodesDiseases = nema || [];
  ...
  return (
    <div className="nematodes-diseases-page  " dir="rtl">
      {/* خلفية زخرفية */}
      <div className="nematodes-bg">
        <div className="bg-glow-brown"></div>
        <div className="bg-particles"></div>
        <div className="bg-nematode-shapes"></div>
      </div>

      {/* رأس الصفحة */}
      <div className="nematodes-header special-page-header">
        ...
        <div className="header-content">
          ...
          <div className="header-text">
            <h1>أمراض النيماتودا</h1>
            <p className="header-en">Nematode Diseases</p>
            <p className="header-description">
              النيماتودا (الديدان الخيطية) كائنات حيوانية دقيقة تصيب الجذور، تسبب تضخمات وتقرحات، 
              وتُهيئ مداخل للفطريات الممرضة. من أكثر الكائنات انتشاراً في التربة المصرية.
            </p>
            <div className="stats-badge">
              <span className="stat-badge">📊 {nematodesDiseases.length} مرض نيماتودي</span>
              <span className="stat-badge">🌱 Meloidogyne • Pratylenchus • Heterodera</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="diseases-stats-footer">
        <div className="stat-card-footer">
          <span className="stat-number-footer">{nematodesDiseases.length}</span>
          <span className="stat-label-footer">مرض نيماتودي</span>
        </div>
        <div className="stat-divider-footer"></div>
        <div className="stat-card-footer">
          <span className="stat-number-footer">10+</span>
          <span className="stat-label-footer">جنس نيماتودا</span>
        </div>
        <div className="stat-divider-footer"></div>
        <div className="stat-card-footer">
          <span className="stat-number-footer">🐛</span>
          <span className="stat-label-footer">مركبات مرضية</span>
        </div>
      </div>
        ...
        <div className="modal-overlay" onClick={closeModal}>
          <div className="disease-modal glass" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
              ...
              {/* الأعداء الطبيعية */}
              {selectedDisease.naturalEnemies && (
                <div className="modal-section">
...
export default NematodesDiseasesPage;