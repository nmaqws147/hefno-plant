...
const AdminBlogPage = () => {
  ...
  useEffect(() => {
    if (showKeyModal && inputRef.current) {
  ...
  const handleSave = async (postData) => {
    try {
      if (postData.id) {
        const data = await api('/api/blog', { method: 'PUT', body: JSON.stringify(postData) });
        toast.success('Post updated');
        setSelected(data.post);
      } else {
        const data = await api('/api/blog', { method: 'POST', body: JSON.stringify(postData) });
    ...
    <>
      {showKeyModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 p-8 w-full max-w-md mx-4 animate-[slideUp_0.3s_ease]">
            <div className="text-center mb-6">
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Enter your admin key to manage posts</p>
            </div>
            <form onSubmit={handleKeySubmit}>
              <div className="mb-4">
                <input
                  ...
                  className={`w-full px-4 py-3 text-sm border rounded-xl bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-gray-100 placeholder-gray-400 focus:ring-2 focus:ring-emerald-500/40 focus:border-emerald-500 outline-none transition-all ${
                    keyError ? 'border-red-400 dark:border-red-500' : 'border-gray-200 dark:border-gray-600'
                  }`}
                />
                {keyError && (
              ...
              <button
                type="submit"
                className="w-full px-4 py-3 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-semibold rounded-xl transition-all shadow-sm hover:shadow-md active:scale-[0.98]"
              >
